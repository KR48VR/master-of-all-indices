...
centric products, which is why nine Asian cities sit over the coverage floor
(see ROSTER.md, "Wave 2 outcome"). Phase 3 therefore favours globally-covered
sources: a measure that blanks the same fifteen non-OECD cities again was
graded down even when technically fillable.
...
| TAL-03 | Knowledge workers | **REJECT** | Eurostat only (EU-only) | 23/55 | — |
...
550 (~99% coverage of the new set — the Asian cities gain the most), lifting
the whole dataset from 922/1045 (88%) to roughly 1468/1595 (~92%) across 29
sourced measures** (~1523/1650 if D3 adopts ODIN as an 11th). Every
floor-breach city gains 8–9 filled measures.
...
- **Source**: `numbeo.com/crime/rankings.jsp` (2026 Mid-Year edition; the
  `?title=2026-mid` archived URL serves the identical table — pin that in
  provenance). 401 cities; all 55 present after name disambiguation.
...
- Spot checks match Numbeo's known pattern (Abu Dhabi 88.6 famously first;
  Manila 35.4 low). Auckland reads oddly low (49.4) — genuine crowd
  perception, covered by the existing "soft signal" caveat.
...
- **Curation is mandatory**: naive name-matching picked a tiny "Bangkok Noi"
  district (river flood "Very low"); the correct region reads High. A
  half-day manual pass over all 55 division choices, recorded per value.
  Known quirks: Copenhagen under "Koebenhavn", Helsinki at province level,
  Taipei only at Taiwan level (country proxy for that one city), heat "No
  Data" for the small Sydney/Melbourne divisions (use ADM1 or blank).
...
- **Source**: IMD SCI free report PDF — per-city survey score for the
  ...
  principled blank). Use the current 2026 edition at integration.
...
- **Osaka is a principled blank**: absent from the rankings because it fails
  Numbeo's own minimum-contributor bar; its low-sample city page (~7–14
  respondents) must NOT be used to backfill.
...
- Spot checks plausible: Delhi 57, Mumbai 55, Jakarta 54 at the top;
  Copenhagen 28 at the bottom.
...
proxy, expert-assessed"), and its exact CC license variant needs one
confirmation before data lands. Otherwise: principled blank for the measure.
...
per-city 5-yr windows that vary by country (disclosed per value).
...
- **LIV-08 Cultural venues**: the crowd-bias risk was confirmed with numbers,
  not hypothetically — Wikidata counts Paris at 1.8× its official museum
  count while counting Shanghai at 0.4× (a ~4.5× relative bias: pure editing
...
- **TAL-03 Knowledge workers**: only Eurostat harmonizes it — 23/55,
  EU-only, London frozen at 2019. The ILO occupational fallback changes both
  level and concept.
...
- **CON-02 Global accessibility**: four independent disqualifiers — GPCI no
  ...
  are in the paid Yearbook; its license forbids reproduction; ≤40/55.
